...
- **InvoiceTypeCode is a strict GİB code list**: `TICARI` and `KDV_MUAFIYETI` are **invalid** (schema control fails). `TICARIFATURA` is a `ProfileID`, not a type. Valid types: `SATIS, IADE, ISTISNA, TEVKIFAT, IHRACAT, …`. Builder constants: `TYPE_TICARI='SATIS'`, `TYPE_KDV_MUAFIYETI='ISTISNA'`. Invoices with VAT-exempt/exempt-code lines must use one of the types in `MUAFIYET_UYUMLU_TIPLER`.
