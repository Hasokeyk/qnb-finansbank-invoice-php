...
- **InvoiceTypeCode sıkı GİB kod listesidir**: `TICARI` ve `KDV_MUAFIYETI` geçersiz (`schematronControl failed: Geçersiz cbc:InvoiceTypeCode`). `TICARIFATURA` bir `ProfileID`'dir, tip değil. Geçerli tipler: `SATIS, IADE, ISTISNA, TEVKIFAT, IHRACAT, ...`. KDV muafiyet kodu `301` olan fatura tipi `ISTISNA` (veya `IADE,IHRACKAYITLI,SGK,YTBIADE,YTBISTISNA`) olmalıdır. Builder sabitleri: `TYPE_TICARI='SATIS'`, `TYPE_KDV_MUAFIYETI='ISTISNA'`.
