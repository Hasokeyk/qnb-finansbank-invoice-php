...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    "substring-after($pText, '&#xA;')"/>
		...
		<xsl:choose>
			...
			<xsl:when test="$CountryType='XK'">Kosova</xsl:when>
			<xsl:when test="$CountryType='CTR'">KKTC</xsl:when>
		...
		<td style="width:469px; " align="left">
			...
			<xsl:for-each select="cac:Person">
				...
				<xsl:if test="$PartyType='TAXFREE'">
					...
					<br />
					<xsl:if test="cac:FinancialAccount/cbc:ID !=''">
						<xsl:text>Turist Banka Hesap No: </xsl:text>
						<xsl:value-of select="cac:FinancialAccount/cbc:ID" />
						<br />
					</xsl:if>
					<xsl:if test="cac:FinancialAccount/cac:FinancialInstitutionBranch/cac:FinancialInstitution/cbc:Name !=''">
						<xsl:text>Banka Adı: </xsl:text>
						<xsl:value-of select="cac:FinancialAccount/cac:FinancialInstitutionBranch/cac:FinancialInstitution/cbc:Name" />
						<br />
					</xsl:if>
					<xsl:if test="cac:FinancialAccount/cac:FinancialInstitutionBranch/cbc:Name !=''">
						<xsl:text>Banka Şubesi: </xsl:text>
						<xsl:value-of select="cac:FinancialAccount/cac:FinancialInstitutionBranch/cbc:Name"/>
						<br />
					</xsl:if>
					<xsl:if test="cac:FinancialAccount/cbc:PaymentNote !=''">
						<xsl:text>Ödeme Notu: </xsl:text>
						<xsl:value-of select="cac:FinancialAccount/cbc:PaymentNote" />
						<br />
					</xsl:if>
					<xsl:if test="//n1:Invoice/cac:BuyerCustomerParty/cac:Party/cac:Person/cac:FinancialAccount/cbc:CurrencyCode !=''">
						<xsl:text>Para Birimi: </xsl:text>
						<xsl:if test="//n1:Invoice/cac:BuyerCustomerParty/cac:Party/cac:Person/cac:FinancialAccount/cbc:CurrencyCode">
							<xsl:text></xsl:text>
							<xsl:if test="cac:FinancialAccount/cbc:CurrencyCode = 'TRY' or cac:FinancialAccount/cbc:CurrencyCode = 'TRL'">
								<xsl:text>TL</xsl:text>
							</xsl:if>
							<xsl:if test="cac:FinancialAccount/cbc:CurrencyCode != 'TRY' and cac:FinancialAccount/cbc:CurrencyCode != 'TRL'">
								<xsl:value-of select="cac:FinancialAccount/cbc:CurrencyCode"/>
							</xsl:if>
						</xsl:if>
					</xsl:if>
				</xsl:if>
		...
		<td style="width:469px; " align="left">
			<xsl:for-each select="cac:PostalAddress">
				...
				<xsl:for-each select="cbc:CitySubdivisionName">
					<xsl:text>Turist İlçe Adı: </xsl:text>
					<xsl:apply-templates/>
					<xsl:text>/ </xsl:text>
				</xsl:for-each>
				<xsl:for-each select="cbc:CityName">
					<xsl:text>Turist Şehri: </xsl:text>
					<xsl:apply-templates/>
					<xsl:text>&#160;</xsl:text>
				</xsl:for-each>
				<xsl:if test="$PartyType='TAXFREE'">
					<br/>
					<xsl:text>Turistin İkametgah Ülkesi: </xsl:text>
...
<xsl:template match="/">
        <xsl:comment>QNB eSolutions Şablon Tasarım Aracı İle Hazırlanmıştır.</xsl:comment>
...
<style type="text/css">
                                        .documentContainer:hover .qrcode {
                                        ...
                                        .documentContainer #notlar a {pointer-events: auto !important;}
                                        .documentContainer,
                                        ...
                                        #malHizmetTablosu > tbody > tr > th.alignLeft {text-align:left;width: 22%;}
...
<xsl:for-each
        ...
        <xsl:if test="@schemeID != 'MUSTERINO'">
                <xsl:if test="@schemeID = 'SEVKIYATNO' and //n1:Invoice/cbc:ProfileID = 'IDIS'">
                            <div class="partyID">
                                <xsl:text>Sevkiyat No: </xsl:text>
                                <xsl:value-of select="."/>
                            </div>
                        </xsl:if>

                        <xsl:if test="@schemeID != 'SEVKIYATNO'">
                            <div class="partyID">
                                <xsl:choose>
...
<xsl:if test="//n1:Invoice/cbc:ProfileID = 'YOLCUBERABERFATURA'">
    <xsl:for-each select="//n1:Invoice/cac:BuyerCustomerParty/cac:Party/cac:PostalAddress">
		<xsl:for-each select="cbc:StreetName">
			<xsl:apply-templates/>
			<xsl:text>&#160;</xsl:text>
		</xsl:for-each>
...
<div class="ph filled qrcode" style="margin-bottom: -70px;">
        <div id= "QRCODE" class="test kutu">
...
"avkntckn":"<xsl:value-of select="n1:Invoice/cac:AccountingCustomerParty/cac:Party/cac:PartyIdentification/cbc:ID[@schemeID = 'TCKN' or @schemeID = 'VKN']"/>", <xsl:if test="n1:Invoice/cbc:ProfileID ='YOLCUBERABERFATURA'">
...
"kdvmatrah</xsl:text>(<xsl:value-of select="cbc:Percent"/>)":"<xsl:value-of select="cbc:TaxableAmount"/>"</xsl:for-each>
...
"odenecek":"<xsl:value-of select="n1:Invoice/cac:LegalMonetaryTotal/cbc:PayableAmount"/>"}
           ...
           <script type="text/javascript">var qrEl=document.getElementById("qrcode");if(qrEl){qrEl.innerHTML="";var qrcode=new QRCode(qrEl,{width:140,height:140,correctLevel:QRCode.CorrectLevel.M});function makeCode(msg){qrcode.makeCode(msg);}var v=document.getElementById("qrvalue");if(v){makeCode(v.innerHTML);}}</script>
        </div>
</div>
<div class="ph empty ph7">
    
    <xsl:text disable-output-escaping="yes">
        <![CDATA[<img style='display: block; margin-left: auto; margin-right: auto;' src='data:image/png;base64,'/><br/>]]>
    </xsl:text>
</div>
...
<th data-id="SATIRLAR_HARCAMA_TIPI">
        <xsl:text>Harcama Tipi</xsl:text>
</th>
</xsl:if> 
<xsl:if test="//n1:Invoice/cbc:ProfileID = 'YATIRIMTESVIK'">

<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode = '01'">
  <th data-id="SATIRLAR_MAKINE_ADI">
...
<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode = '01'">
          <th data-id="SATIRLAR_MAKINE_TECHIZAT_SIRANO">
...
</th><xsl:if test="//n1:Invoice/cbc:ProfileID='ILAC_TIBBICIHAZ'">

<th data-id="SATIRLAR_ILAC_TIBBICIHAZ">
        <xsl:text>İlaç/Tıbbi Cihaz</xsl:text>
</th></xsl:if> 
<xsl:if test="//n1:Invoice/cbc:ProfileID='ILAC_TIBBICIHAZ'">

<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC']">
<xsl:if test="contains(//cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC'], '(GTIN)')">
												<th data-id="SATIRLAR_ILACKURESELTICARIURUNNO">Küresel Ticari Ürün Numarası</th>
											</xsl:if>
											</xsl:if></xsl:if> 
<xsl:if test="//n1:Invoice/cbc:ProfileID='ILAC_TIBBICIHAZ'">

<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC']">
<xsl:if test="contains(//cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC'], '(BN)')">
												<th data-id="SATIRLAR_ILACPARTINO">Parti Numarası</th>
											</xsl:if>
											</xsl:if></xsl:if> 
<xsl:if test="//n1:Invoice/cbc:ProfileID='ILAC_TIBBICIHAZ'">

<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC']">
<xsl:if test="contains(//cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC'], '(SN)')">
												<th data-id="SATIRLAR_ILACSERINO">Seri Numarası</th>
											</xsl:if>
											</xsl:if></xsl:if> 
<xsl:if test="//n1:Invoice/cbc:ProfileID='ILAC_TIBBICIHAZ'">

<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC']">
<xsl:if test="contains(//cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC'], '(XD)')">
												<th data-id="SATIRLAR_ILACSONKULLANMATARIH">Son Kullanma Tarihi</th>
											</xsl:if>
											</xsl:if></xsl:if> 
<xsl:if test="//n1:Invoice/cbc:ProfileID='ILAC_TIBBICIHAZ'">

<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ']">
<xsl:if test="contains(//cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ'], '(UNO)')">
												<th data-id="SATIRLAR_TIBBICIHAZURUNNO">Ürün Numarası</th>
											</xsl:if>
											</xsl:if></xsl:if> 
<xsl:if test="//n1:Invoice/cbc:ProfileID='ILAC_TIBBICIHAZ'">

<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ']">
<xsl:if test="contains(//cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ'], '(LNO)')">
												<th data-id="SATIRLAR_TIBBICIHAZLOTNO">Lot/Batch Numarası</th>
											</xsl:if>
											</xsl:if></xsl:if> 
<xsl:if test="//n1:Invoice/cbc:ProfileID='ILAC_TIBBICIHAZ'">

<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ']">
<xsl:if test="contains(//cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ'], '(SNO)')">
												<th data-id="SATIRLAR_TIBBICIHAZSERISIRANO">Seri/Sıra Numarası</th>
											</xsl:if>
											</xsl:if></xsl:if> 
<xsl:if test="//n1:Invoice/cbc:ProfileID='ILAC_TIBBICIHAZ'">

<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ']">
<xsl:if test="contains(//cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ'], '(URT)')">
												<th data-id="SATIRLAR_TIBBICIHAZURETIMTARIH">Üretim Tarihi</th>
											</xsl:if>
											</xsl:if></xsl:if> 
...
<th data-id="SATIRLAR_ISKONTOTUTARI">
        <xsl:text>İskonto / Artırım Tutarı</xsl:text>
</th>
</xsl:if></xsl:if> 
...
<xsl:if test="not(//n1:Invoice/cbc:InvoiceTypeCode = 'ISTISNA' and //n1:Invoice/cbc:ProfileID = 'YATIRIMTESVIK')">
<xsl:if test="//n1:Invoice/cbc:ProfileID!='IHRACAT' and //n1:Invoice/cac:InvoiceLine/cac:TaxTotal/cac:TaxSubtotal/cbc:Percent >0">
...
<xsl:if test="//n1:Invoice/cbc:ProfileID = 'IDIS'">

        <xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ETIKETNO']">
        <th data-id="SATIRLAR_ETIKETNO">
            <xsl:text>Etiket No</xsl:text>
        </th>
        </xsl:if></xsl:if> 
...
<xsl:if test="//n1:Invoice/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReasonCode='702' and //n1:Invoice/cbc:InvoiceTypeCode='IHRACKAYITLI'">
											<th data-id="SATIRLAR_GTIP">
												<xsl:text>GTİP</xsl:text>
											</th>
											</xsl:if>
</xsl:if> 
<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Delivery/cac:Shipment/cac:TransportHandlingUnit/cac:CustomsDeclaration/cac:IssuerParty/cac:PartyIdentification/cbc:ID[@schemeID='ALICIDIBSATIRKOD']">

	<xsl:if test="//n1:Invoice/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReasonCode='702' and //n1:Invoice/cbc:InvoiceTypeCode='IHRACKAYITLI'">


											<th data-id="SATIRLAR_ALICISATIRKODU">
												<xsl:text>Alıcı Satır Kodu</xsl:text>
											</th>
											</xsl:if>

</xsl:if> 
<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Delivery/cac:Shipment/cac:TransportHandlingUnit/cac:CustomsDeclaration/cac:IssuerParty/cac:PartyIdentification/cbc:ID[@schemeID='SATICIDIBSATIRKOD']">

	<xsl:if test="//n1:Invoice/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReasonCode='702' and //n1:Invoice/cbc:InvoiceTypeCode='IHRACKAYITLI'">


											<th data-id="SATIRLAR_SATICISATIRKODU">
...
<xsl:if test="//n1:Invoice/cbc:ProfileID='IHRACAT'">
		<tr>
                        ...
                        <tr>
                             <xsl:choose>
                                <xsl:when test="//n1:Invoice/cbc:AccountingCost='SAGLIK_OPT'">
                                    <th>
...
<xsl:if test="not(//n1:Invoice/cbc:InvoiceTypeCode = 'ISTISNA' and //n1:Invoice/cbc:ProfileID = 'YATIRIMTESVIK')">
              ...
              <xsl:if test="not(//n1:Invoice/cbc:InvoiceTypeCode = 'ISTISNA' and //n1:Invoice/cbc:ProfileID = 'YATIRIMTESVIK')">
                ...
                <xsl:if test="n1:Invoice/cac:WithholdingTaxTotal/cac:TaxSubtotal[starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'6') or (starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'8'))]">
                        <tr>
                                <th class="sumTitle">
                                        <xsl:text>Tevkifata Tabi İşlem Tutarı (KDV): </xsl:text>
                                </th>
                                <td class="sumValue">
                                        <xsl:value-of select="format-number(sum(n1:Invoice/cac:InvoiceLine/cac:TaxTotal/cac:TaxSubtotal/cbc:TaxableAmount[../../../cac:WithholdingTaxTotal/cac:TaxSubtotal[starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'6') or (starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'8'))] and ../cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode =0015]), '###.##0,00', 'european')"/>
                                        <span>
                                                <xsl:if test="//n1:Invoice/cbc:DocumentCurrencyCode = 'TRY' or //n1:Invoice/cbc:DocumentCurrencyCode = 'TRL'">
                                                        <xsl:text> TL</xsl:text>
                                                </xsl:if>
				...
				<xsl:if test="n1:Invoice/cac:WithholdingTaxTotal/cac:TaxSubtotal[starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'6') or (starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'8'))]">
                        <tr>
                                ...
                                <td class="sumValue">
                                        <xsl:value-of select="format-number(sum(n1:Invoice/cac:WithholdingTaxTotal/cac:TaxSubtotal/cbc:TaxableAmount[starts-with(../cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'6') or (starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'8'))]), '###.##0,00', 'european')"/>
                                        <span>
                                                <xsl:if test="//n1:Invoice/cbc:DocumentCurrencyCode = 'TRY' or //n1:Invoice/cbc:DocumentCurrencyCode = 'TRL'">
                                                        <xsl:text> TL</xsl:text>
                                                </xsl:if>
                ...
                <xsl:if test="n1:Invoice/cac:WithholdingTaxTotal/cac:TaxSubtotal[starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'6') or (starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'8'))]">
                        ...
                        <xsl:for-each select="n1:Invoice/cac:WithholdingTaxTotal/cac:TaxSubtotal[starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'6') or (starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'8'))]">
                                <tr>
                                        <th>
...
<xsl:if test="//n1:Invoice/cbc:ProfileID != 'YOLCUBERABERFATURA' and //n1:Invoice/cbc:ProfileID = 'IHRACAT'">
						<xsl:if test="
    //n1:Invoice/cac:Delivery/cac:DeliveryTerms/cbc:ID[@schemeID='INCOTERMS'] != ''
    or //n1:Invoice/cac:Delivery/cac:Shipment/cac:ShipmentStage/cbc:TransportModeCode != ''
    or //n1:Invoice/cac:Delivery/cac:Shipment/cac:GoodsItem/cbc:RequiredCustomsID != ''
    or //n1:Invoice/cac:Delivery/cac:Shipment/cac:TransportHandlingUnit/cac:ActualPackage/cbc:PackagingTypeCode != ''
    or normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cac:Country/cbc:Name) != ''
    or normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:CityName) != ''
    or normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:CitySubdivisionName) != ''
    or normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:PostalZone) != ''
    or normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:StreetName) != ''
    or normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:BuildingNumber) != ''
    or normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:BuildingName) != ''
...
">
    ...
    ">
								...
								<xsl:if test=" normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cac:Country/cbc:Name) != ''
        or normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:CityName) != ''
        or normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:CitySubdivisionName) != ''
        or normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:PostalZone) != ''
		or normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:StreetName) != ''
    ...
    ">
       ...
       //n1:Invoice/cac:Delivery/cac:DeliveryTerms/cbc:ID[@schemeID='INCOTERMS'] != ''">
											...
											<xsl:if test="
        //n1:Invoice/cac:Delivery/cac:Shipment/cac:ShipmentStage/cbc:TransportModeCode != ''">
												<br/>
												<xsl:text>Gönderilme Şekli: </xsl:text>
											...
											<xsl:if test="
        //n1:Invoice/cac:Delivery/cac:Shipment/cac:GoodsItem/cbc:RequiredCustomsID != ''">
												<br/>
											...
											<xsl:if test="
        //n1:Invoice/cac:Delivery/cac:Shipment/cac:TransportHandlingUnit/cac:ActualPackage/cbc:PackagingTypeCode != ''">
												<br/>
												<xsl:text>Eşya Kap Cinsi: </xsl:text>
		...
		">
											<td vaign="top" width="50%">
												...
												<xsl:if test="normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:CityName) != ''">
													<br/>
												...
												<xsl:if test="normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:CitySubdivisionName) != ''">
													<br/>
												...
												<xsl:if test="normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:PostalZone) != ''">
													<br/>
												...
												<xsl:if test="normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:StreetName) != ''">
													<br/>
													<xsl:text>Sokak: </xsl:text>
												...
												<xsl:if test="normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:BuildingNumber) != ''">
													<br/>
													<xsl:text>No: </xsl:text>
												...
												<xsl:if test="normalize-space(//n1:Invoice/cac:Delivery/cac:DeliveryAddress/cbc:BuildingName) != ''">
													<br/>
...
<div id="notlar">
 ...
 <xsl:for-each select="//n1:Invoice/cbc:Note">
    <xsl:choose>
       ...
       <xsl:when test="1=1 and not(contains(., 'Ödeme Linki')) and not(starts-with(.,'#EFN'))">
            <xsl:value-of select="." disable-output-escaping="yes"/>
            <br/>
        </xsl:when>

        
        <xsl:when test="contains(., 'Ödeme Linki---')">
            <xsl:variable name="url" select="normalize-space(substring-after(., 'Ödeme Linki---'))"/>
            <xsl:variable name="prefix" select="normalize-space(substring-before(., 'Ödeme Linki---'))"/>

            <p style="margin:0; padding:0;">
                <b>*</b>
                <xsl:value-of select="$prefix"/>
                <xsl:text> Ödeme Linki: </xsl:text>
...
<xsl:if test="//n1:Invoice/cac:AdditionalDocumentReference[cbc:DocumentType!='XSLT']">
    <div style="margin-top:6px;">
        <span style="font-weight:bold;">İLAVE DÖKÜMANLAR</span>
        <table style="width:100%; border-collapse:collapse; margin-top:4px;">
            <xsl:for-each select="//n1:Invoice/cac:AdditionalDocumentReference[cbc:DocumentType!='XSLT']">
                <tr>
                   <td style="padding:0;">
                            <xsl:if test="cbc:ID">
                        ...
                        <xsl:if test="cbc:DocumentTypeCode">
                            <span style="font-weight:bold;">Belge Kodu:</span>
                            <xsl:value-of select="cbc:DocumentTypeCode"/>
                            <xsl:text> </xsl:text>
                        </xsl:if>
                        <xsl:if test="cbc:DocumentDescription">
                            <span style="font-weight:bold;">Belge Açıklaması:</span>
                                                ...
                                                <tbody>
                                                    <xsl:for-each select="/n1:Invoice/cac:InvoiceLine">
                                                        <tr>
                                                            ...
                                                            <td class="wrap" style="word-break: break-all;overflow-wrap: anywhere;">
                                                                    ...
                                                                    <xsl:if test="./cbc:InvoicedQuantity/@unitCode">
                                                                        <xsl:for-each select="./cbc:InvoicedQuantity">
                                                                            ...
                                                                            <xsl:choose>
                                                                                ...
                                                                                <xsl:when test="@unitCode = 'AO'">
                                                                                    <span>
                                                                                        <xsl:text> Alkol Oranı/Derecesi</xsl:text>
                                                                                    </span>
                                                                                </xsl:when>
                                                                                ...
                                                                                <xsl:when test="@unitCode = 'K3'">
                                                                                    <span>
                                                                                        <xsl:text> Kilovolt Amper Reaktif Saat</xsl:text>
                                                                                    </span>
                                                                                </xsl:when>
                                                                                ...
                                                                                <xsl:when test="@unitCode = 'DR' or @unitCode = '2W'">
                                                                                    <span>
                                                                                ...
                                                                                <xsl:when test="@unitCode = 'A45'">
                                                                                    <span>
                                                                                        <xsl:text> Dekametre</xsl:text>
                                                                                    </span>
                                                                                </xsl:when>
                                                                                  <xsl:when test="@unitCode = 'TS'">
                                                                                    <span>
                                                            ...
                                                            <td>
                                                                <xsl:if test="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC']">
                                                                        <span>
                                                                            <xsl:text>İlaç</xsl:text>
                                                                        </span>
                                                                </xsl:if>
                                                                <xsl:if test="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ']">
                                                                        <span>
                                                                            <xsl:text>Tıbbi Cihaz</xsl:text>
                                                                        </span>
                                                                </xsl:if>
                                                                <xsl:if test="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='DIGER']">
                                                                        <span>
                                                                            <xsl:text>Diğer</xsl:text>
                                                                        </span>
                                                                </xsl:if>
                                                            </td>
                                                            <td class="lineTableTd" align="left" style="word-wrap: break-word; overflow-wrap: anywhere; white-space: normal;">
                                                            <xsl:for-each select="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ' or @schemeID='ILAC' or @schemeID='DIGER']">
                                                                    <div>
                                                                        <xsl:value-of select="."/>
                                                                    </div>
                                                                </xsl:for-each>
...
<td class="wrap" style="word-wrap: break-word; overflow-wrap: break-word; word-break: normal;">
        <span>
                <xsl:value-of select="./cac:Item/cbc:Name"/>
                <xsl:text>&#160;</xsl:text>
                <xsl:value-of select="./cac:Item/cbc:BrandName"/>
                <xsl:text>&#160;</xsl:text>
                <xsl:if test="not(//n1:Invoice/cbc:ProfileID = 'YATIRIMTESVIK')">
				...
				<xsl:if test="cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode=01">
					<td class="lineTableTd" align="rigth">
						<span>
							<xsl:text>01-Makine ve teçhizat teslimleri ile yazılım ve gayrimaddi hak satış ve kiralamaları</xsl:text>
						</span>
					</td>
				</xsl:if>
				<xsl:if test="cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode=02">
					<td class="lineTableTd" align="right">
						<span>
							<xsl:text>02-İnşaat işlerine ilişkin mal teslimleri ve hizmet ifaları</xsl:text>
						</span>
					</td>
				</xsl:if>
				<xsl:if test="cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode=03">
					<td class="lineTableTd" align="right">
						<span>
							<xsl:text>03-Arsa /Arazi Satışları</xsl:text>
						</span>
					</td>
				</xsl:if>
				<xsl:if test="cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode=04">
					<td class="lineTableTd" align="right">
						<span>
							<xsl:text>04-Diğer harcamalar</xsl:text>
						</span>
					</td>
				</xsl:if>
...
<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:CommodityClassification/cbc:ItemClassificationCode = '01'">
<td class="lineTableTd" align="right" style="word-break: break-all; white-space: normal;">
					<span>
						<xsl:choose>
							<xsl:when test="./cac:Item/cbc:ModelName != ''">
								<xsl:value-of select="cac:Item/cbc:ModelName"/>
							</xsl:when>
...
<td class="lineTableTd" align="right" style="word-break: break-all; white-space: normal;">
 					<span>
 						<xsl:choose>
 							<xsl:when test="./cac:Item/cac:ItemInstance/cbc:ProductTraceID[normalize-space(.) != '']">
 								<xsl:value-of select="./cac:Item/cac:ItemInstance/cbc:ProductTraceID"/>
 							</xsl:when>
 							<xsl:otherwise>-</xsl:otherwise>
 						</xsl:choose>
 					</span>
...
<td class="lineTableTd" align="right" style="word-break: break-all; white-space: normal;">
					<span>
						<xsl:choose>
							<xsl:when test="./cac:Item/cac:ItemInstance/cbc:SerialID[normalize-space(.) != '']">
								<xsl:value-of select="cac:Item/cac:ItemInstance/cbc:SerialID"/>
							</xsl:when>
							<xsl:otherwise>-</xsl:otherwise>
						</xsl:choose>
...
<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC']">
<xsl:if test="contains(//cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC'], '(GTIN)')">
<td class="lineTableTd" style="word-break: break-all; white-space: normal;">
        <span style="display: block; width: 100%; text-align: center;">
            <xsl:choose>
                <xsl:when test="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC' and contains(., '(GTIN)')]">
                    <xsl:for-each select="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC' and contains(., '(GTIN)')]">
                        <xsl:value-of select="normalize-space(substring-before(substring-after(., '(GTIN)'), '(BN)'))"/>
                        <br/>
                    </xsl:for-each>
                </xsl:when>
                <xsl:otherwise>-</xsl:otherwise>
...
<td class="lineTableTd" style="word-break: break-all; white-space: normal;">
        <span style="display: block; width: 100%; text-align: center;">
            <xsl:choose>
                <xsl:when test="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC' and contains(., '(BN)')]">
                    <xsl:for-each select="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC' and contains(., '(BN)')]">
                        <xsl:value-of select="normalize-space(substring-before(substring-after(., '(BN)'), '(SN)'))"/>
                        <br/>
                    </xsl:for-each>
                </xsl:when>
                <xsl:otherwise>-</xsl:otherwise>
...
<td class="lineTableTd" style="word-break: break-all; white-space: normal;">
        <span style="display: block; width: 100%; text-align: center;">
            <xsl:choose>
                <xsl:when test="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC' and contains(., '(SN)')]">
                    <xsl:for-each select="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC' and contains(., '(SN)')]">
                        <xsl:value-of select="normalize-space(substring-before(substring-after(., '(SN)'), '(XD)'))"/>
                        <br/>
                    </xsl:for-each>
                </xsl:when>
                <xsl:otherwise>-</xsl:otherwise>
...
<td class="lineTableTd" style="word-break: break-all; white-space: normal;">
        <span style="display: block; width: 100%; text-align: center;">
            <xsl:choose>
                <xsl:when test="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC' and contains(., '(XD)')]">
                    <xsl:for-each select="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ILAC' and contains(., '(XD)')]">
                        <xsl:value-of select="normalize-space(substring-after(., '(XD)'))"/>
                        <br/>
                    </xsl:for-each>
                </xsl:when>
                <xsl:otherwise>-</xsl:otherwise>
...
<xsl:if test="//n1:Invoice/cbc:ProfileID='ILAC_TIBBICIHAZ'">
 ...
 <td class="lineTableTd" style="word-break: break-all; white-space: normal;">
        <span style="display: block; width: 100%; text-align: center;">
            <xsl:choose>
                <xsl:when test="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ' and contains(., '(UNO)')]">
                    <xsl:for-each select="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ' and contains(., '(UNO)')]">
                        <xsl:value-of select="normalize-space(substring-before(substring-after(., '(UNO)'), '(LNO)'))"/>
                        <br/>
                    </xsl:for-each>
                </xsl:when>
                <xsl:otherwise>-</xsl:otherwise>
...
<xsl:if test="//n1:Invoice/cbc:ProfileID='ILAC_TIBBICIHAZ'">
 ...
 <td class="lineTableTd" style="word-break: break-all; white-space: normal;">
        <span style="display: block; width: 100%; text-align: center;">
            <xsl:choose>
                <xsl:when test="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ' and contains(., '(LNO)')]">
                    <xsl:for-each select="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ' and contains(., '(LNO)')]">
                        <xsl:value-of select="normalize-space(substring-before(substring-after(., '(LNO)'), '(SNO)'))"/>
                        <br/>
                    </xsl:for-each>
                </xsl:when>
                <xsl:otherwise>-</xsl:otherwise>
...
<td class="lineTableTd" style="word-break: break-all; white-space: normal;">
        <span style="display: block; width: 100%; text-align: center;">
            <xsl:choose>
                <xsl:when test="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ' and contains(., '(SNO)')]">
                    <xsl:for-each select="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ' and contains(., '(SNO)')]">
                        <xsl:value-of select="normalize-space(substring-before(substring-after(., '(SNO)'), '(URT)'))"/>
                        <br/>
                    </xsl:for-each>
                </xsl:when>
                <xsl:otherwise>-</xsl:otherwise>
...
<xsl:if test="//n1:Invoice/cbc:ProfileID='ILAC_TIBBICIHAZ'">
 ...
 <td class="lineTableTd" style="word-break: break-all; white-space: normal;">
        <span style="display: block; width: 100%; text-align: center;">
            <xsl:choose>
                <xsl:when test="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ' and contains(., '(URT)')]">
                    <xsl:for-each select="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='TIBBICIHAZ' and contains(., '(URT)')]">
                        <xsl:value-of select="normalize-space(substring-after(., '(URT)'))"/>
                        <br/>
                    </xsl:for-each>
                </xsl:when>
                <xsl:otherwise>-</xsl:otherwise>
...
<xsl:if test="//n1:Invoice/cbc:ProfileID = 'IDIS' ">
				...
				<xsl:if test="//n1:Invoice/cac:InvoiceLine/cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ETIKETNO']">
				 <td>
					<xsl:value-of select="cac:Item/cac:AdditionalItemIdentification/cbc:ID[@schemeID='ETIKETNO']"/>
				 </td>
				</xsl:if></xsl:if> 
<xsl:if test="//n1:Invoice/cbc:ProfileID='IHRACAT'">
...
<xsl:if test="//n1:Invoice/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReasonCode='702' and //n1:Invoice/cbc:InvoiceTypeCode='IHRACKAYITLI'">
				...
				<td>
					<xsl:value-of select="//n1:Invoice/cac:InvoiceLine/cac:Delivery/cac:Shipment/cac:GoodsItem/cbc:RequiredCustomsID"/>
				</td>
				</xsl:if>
...
<xsl:if test="//n1:Invoice/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReasonCode='702' and //n1:Invoice/cbc:InvoiceTypeCode='IHRACKAYITLI'">
				...
				<td>
					<xsl:value-of select="//n1:Invoice/cac:InvoiceLine/cac:Delivery/cac:Shipment/cac:TransportHandlingUnit/cac:CustomsDeclaration/cac:IssuerParty/cac:PartyIdentification/cbc:ID[@schemeID='ALICIDIBSATIRKOD']"/>
				</td>
				</xsl:if>
...
<xsl:if test="//n1:Invoice/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReasonCode='702' and //n1:Invoice/cbc:InvoiceTypeCode='IHRACKAYITLI'">
				...
				<td>
					<xsl:value-of select="//n1:Invoice/cac:InvoiceLine/cac:Delivery/cac:Shipment/cac:TransportHandlingUnit/cac:CustomsDeclaration/cac:IssuerParty/cac:PartyIdentification/cbc:ID[@schemeID='SATICIDIBSATIRKOD']"/>
				</td>
				</xsl:if>
...
<td style="font-size:xx-small;">
        <div>
                <xsl:for-each
                        ...
                        <xsl:if test="cbc:TaxTypeCode!='0015' ">
                                <span class="is-long-{string-length(cbc:Name) > 15}">
                                        <xsl:text> </xsl:text>
                                        <xsl:if test="starts-with(cbc:TaxTypeCode,'4') or starts-with(cbc:TaxTypeCode,'6') or (starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'8'))">
                ...
                <xsl:for-each
                        ...
                        <xsl:if test="cbc:TaxTypeCode!='0015' ">
                                <span class="is-long-{string-length(cbc:Name) > 15}"><xsl:if test="starts-with(cbc:TaxTypeCode,'4') or starts-with(cbc:TaxTypeCode,'6') or (starts-with(./cac:TaxCategory/cac:TaxScheme/cbc:TaxTypeCode,'8'))">
                                                <xsl:value-of select="cbc:TaxTypeCode"/> -
                                        </xsl:if>
                                        <xsl:value-of select="cbc:Name"/>
                                </span>
